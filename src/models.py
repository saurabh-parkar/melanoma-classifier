    ...
    def load(self):
        try:
            if os.path.exists(self.weights):
                print(self.weights)
                if self.weights.find('pw') != -1:
                    print('Loading "patch-wise" model...', self.weights)
                elif self.weights.find('iw') != -1:
                    print('Loading "image-wise" model...', self.weights)
...
class PatchWiseModel(BaseModel):
    def __init__(self, args, network):
        super(PatchWiseModel, self).__init__(args, network, args.checkpoints_path + '/weights_pw_1.pth')
...
class ImageWiseModel(BaseModel):
    def __init__(self, args, image_wise_network, patch_wise_network):
        super(ImageWiseModel, self).__init__(args, image_wise_network, args.checkpoints_path + '/weights_iw_0107.pth')
    ...
    def train(self):
        ...
        optimizer = optim.Adam(self.network.parameters(), lr=self.args.lr, betas=(self.args.beta1, self.args.beta2))
        best, best_loss = self.validate(verbose=False)
        mean = 0
        epoch = 0
